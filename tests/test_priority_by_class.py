...
from ra_utils.priority_by_class import lc_choose_public_address
...
class PriorityByClassTests(TestCase):
    def setUp(self):
        ...
        self.candidates = list(
            map(itemgetter(0), self.lc.addresses.values())  # type: ignore
        )

    def get_candidate_uuid(self, priority, reverse=False, shuffled=False):
        candidates = self.candidates
        if reverse:
            candidates = reversed(candidates)  # type: ignore
        if shuffled:
            tmp_list = candidates[:]
            shuffle(tmp_list)
            candidates = tmp_list
